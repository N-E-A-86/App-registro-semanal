...
let formularioHoras, formularioPlan, listaTareas, totalHorasElement, totalHorasMesElement, porcentajeProgresoElement, tareasCompletadasElement, totalTareasElement, progresoFill;

// Variable para el historial mensual
let historialMesSeleccionado = null;

/**
 * Parsea una cadena de fecha YYYY-MM-DD como una fecha local a mediodía.
...
function setHistorialMes(mesOffset) {
    ...
    historialMesSeleccionado = { mes: fechaRef.getMonth(), año: fechaRef.getFullYear() };
...
function calcularHorasSemanales() {
    ...
    window.horasTrabajadas.forEach(registro => {
        ...
        if (fechaRegistro && fechaRegistro >= inicioSemana && fechaRegistro <= finSemana) {
    ...
    totalHorasElement.textContent = (totalMinutos / 60).toFixed(2);
}

function calcularHorasMensuales() {
    if (!totalHorasMesElement) return;
    ...
    window.horasTrabajadas.forEach(registro => {
        const fechaRegistro = parseDateAsLocal(registro.fecha);
        if (fechaRegistro && fechaRegistro.getMonth() === mesActual && fechaRegistro.getFullYear() === añoActual) {
            const [hEntrada, mEntrada] = registro.horaEntrada.split(':').map(Number);
            const [hSalida, mSalida] = registro.horaSalida.split(':').map(Number);
            if (isNaN(hEntrada) || isNaN(mEntrada) || isNaN(hSalida) || isNaN(mSalida)) return;
    ...
    totalHorasMesElement.textContent = (totalMinutos / 60).toFixed(2);
}
...
function renderizarHistorialHoras() {
    ...
    registrosSemana.forEach(registro => {
        ...
        const salida = new Date(`1970-01-01T${registro.horaSalida}`);
...
function agregarHora(evento) {
    ...
    if (new Date(`1970-01-01T${horaSalida}`) <= new Date(`1970-01-01T${horaEntrada}`)) {
        alert('La hora de salida debe ser mayor a la de entrada.');
        return;
    }

    const nuevoRegistro = { id: Date.now(), fecha, horaEntrada, horaSalida };
...
// --- El resto de las funciones (plan de acción, progreso, etc.) ---

function agregarTarea(evento) {
    evento.preventDefault();
    const texto = document.getElementById('tarea').value.trim();
    if (!texto) return alert('Escribe una tarea.');
    window.tareas.push({ id: Date.now(), texto, prioridad: document.getElementById('prioridad').value, completada: false });
...
function renderizarTareas() {
    ...
    if (!window.tareas || window.tareas.length === 0) return;
    window.tareas.forEach(t => {
        const li = document.createElement('li');
        li.className = 'tarea-item';
        li.setAttribute('data-prioridad', t.prioridad);
        li.innerHTML = `<span class="tarea-texto ${t.completada ? 'tarea-completada' : ''}">${t.texto}</span><div class="botones-tarea"><button class="btn btn-completar" onclick="toggleCompletada(${t.id})">${t.completada ? 'Desmarcar' : 'Completar'}</button><button class="btn btn-eliminar" onclick="eliminarTarea(${t.id})">Eliminar</button></div>`;
...
function renderizarHistorialMensual() {
    ...
    if (window.horasTrabajadas.length === 0) return;

    let mesRef = historialMesSeleccionado ? new Date(historialMesSeleccionado.año, historialMesSeleccionado.mes) : new Date();
    const registrosMes = window.horasTrabajadas.filter(r => {
        const fecha = parseDateAsLocal(r.fecha);
        return fecha && fecha.getMonth() === mesRef.getMonth() && fecha.getFullYear() === mesRef.getFullYear();
    });
    ...
    registrosMes.forEach(registro => {
        const duracion = calcularDuracion(registro.horaEntrada, registro.horaSalida);
        if (!duracion) return;
        const li = document.createElement('li');
        li.className = 'registro-hora';
        li.innerHTML = `<div class="detalle"><strong>${registro.horaEntrada}</strong> - <strong>${registro.horaSalida}</strong><br><small>${duracion} | ${formatearFechaLegible(registro.fecha)}</small></div><button class="accion" onclick="eliminarRegistroHora(${registro.id})">Eliminar</button>`;
        lista.appendChild(li);
    });
}
...
function calcularDuracion(entradaStr, salidaStr) {
    ...
    if (isNaN(entrada) || isNaN(salida) || salida <= entrada) return null;
